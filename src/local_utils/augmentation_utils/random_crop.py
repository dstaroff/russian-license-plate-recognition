...
from src.local_utils.bbox_utils import iou
...
def random_crop(
    ...
    for i in range(max_attempts):
        ...
        temp_bboxes[:, :2] = np.maximum(temp_bboxes[:, :2], crop_rect[:,
                                                            :2])  # if bboxes top left is out of crop then use crop's xmin, ymin
